...
from contracts.tool_results import GenericToolResult
...
def tool_result(**fields: Any) -> dict[str, Any]:
    return GenericToolResult(**fields).to_payload()
...
def run_shell(command: str) -> dict[str, Any]:
    try:
        ...
        return tool_result(status='error', command=command, error=str(exc))
...
def read_file(path: str) -> dict[str, Any]:
    try:
        full = safe_path(path)
        if not full.exists():
            return tool_result(status='error', path=path, error='file does not exist')
        if full.is_dir():
            return tool_result(status='error', path=path, error='path is a directory')
        content, truncated = read_limited_text_file(full)
        return tool_result(status='ok', path=path, content=content, truncated=truncated)
    except Exception as exc:
        return tool_result(status='error', path=path, error=str(exc))
...
def replace_in_file(path: str, search: str, replace: str) -> dict[str, Any]:
    try:
        ...
        if count == 0:
            return tool_result(status='error', path=path, error='search text not found')
        if count > 1:
            return tool_result(
                status='error',
                path=path,
...
def remove_file(path: str) -> dict[str, Any]:
    try:
        ...
        if full.is_dir():
            return tool_result(status='error', path=path, error='path is a directory')
        full.unlink()
        return tool_result(status='ok', path=path, message='file removed')
...
def find_file(name: str, root: str = '.') -> dict[str, Any]:
    try:
        if not isinstance(name, str) or not name.strip():
            return tool_result(status='error', error='name must be a non-empty string')

        name = name.strip()
        if '/' in name or '\\' in name:
            return tool_result(status='error', name=name, error='name must be a filename, not a path')

        root_path = safe_path(root)
        if not root_path.exists():
            return tool_result(status='error', name=name, root=root, error='root does not exist')
        if not root_path.is_dir():
            return tool_result(status='error', name=name, root=root, error='root is not a directory')
        ...
        return tool_result(status='error', name=name, root=root, error=str(exc))
