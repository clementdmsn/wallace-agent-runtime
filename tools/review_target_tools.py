...
from contracts.tool_results import ReviewTarget, ReviewTargetResult
...
def review_target_result(**fields: Any) -> dict[str, Any]:
    return ReviewTargetResult(**fields).to_payload()


def discover_review_targets(root: str = '.', max_files: int = 20) -> dict[str, Any]:
    try:
        if not isinstance(root, str) or not root.strip():
            return review_target_result(status='error', error='root must be a non-empty string')
        if not isinstance(max_files, int) or max_files <= 0:
            return review_target_result(status='error', error='max_files must be a positive integer')

        root_path = safe_path(root)
        if not root_path.exists():
            return review_target_result(status='error', root=root, error='root does not exist')

        if root_path.is_file():
            if not _is_review_target(root_path):
                return review_target_result(
                    status='ok',
                    root=root,
                    count=0,
                    truncated=False,
        ...
        return review_target_result(status='error', root=root, error=str(exc))
